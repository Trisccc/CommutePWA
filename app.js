...
(() => {
  ...
  function start(){
    ...
    if(persist({...state,active:a})){draw();renderAll();$('segmentHeading')?.focus();toast('第 1 段计时已开始')}
  }
  function advance(){
  ...
  const oldSetDirection=setDirection;setDirection=(dir,manual=true)=>{if(active?.schemaVersion===3){if(manual&&dir!==active.direction)toast('记录中保持本次路线方向');return}oldSetDirection(dir,manual);draw()};
  ...
  $('navRecord').onclick=$('startCommuteBtn').onclick;
  ...
  rebuild();if(active?.schemaVersion===3){directionManuallySet=true;oldSetDirection(active.direction,false)}renderAll();updateRecordRoutes();draw();
})();
